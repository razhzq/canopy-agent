...
10. **Multi-timeframe (1 week) — shipped 14 Sep 2026.** A rule carries a `timeframe` beside its period; the bar-based keys read that chart's bars (one extra candle read per chart per asset per tick), served under keys like `smaSpreadPct@tf1d`. The composer runs the strategy on the fastest chart named and tags the rest; the builder chip has a chart picker. A second series per market, facts keyed by
